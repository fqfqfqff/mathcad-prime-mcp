...
колонками. Поверх этого учтены две особенности отрисовки:

- формула привязана по **базовой линии**, поэтому дроби и степени выступают
  вверх за свой `top`, и над рядом резервируется запас;
- `actualWidth` **не включает надстрочную степень**, поэтому значение вида
  `6.554·10^4` рисуется шире, чем сообщает Prime, и наезжает на соседа справа.
  Регионам с экспоненциальным результатом добавляется запас по ширине.

Ряд, который не влезает в ширину листа, переносится на следующую строку сам.
...
- Картинки снимаются в масштабе Prime. Чтобы пикселей было больше без возни
  с зумом, вспомогательные листы печатаются на **A3**: график выходит ~950 px
  вместо ~610. Высота ограничена видимой частью окна, иначе край срежет.
- `mathcad_figures` сам выбрасывает из вспомогательного листа вызовы
  `WRITE*`/`APPEND*`, иначе пересчёт перезаписал бы файлы рабочей лабы.
